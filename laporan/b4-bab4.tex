...
\end{lstlisting}
          \end{minipage}
          \caption{Implementasi ConfigModule sebagai global configuration}
          \label{fig:config-module}
        \end{figure}
...
\end{lstlisting}
          \end{minipage}
          \caption{Instalasi package LangChain.js dan dependensi pendukung}
          \label{fig:install-package}
        \end{figure}
...
\end{lstlisting}
          \end{minipage}
          \caption{Instalasi PostgreSQL driver}
          \label{fig:install-postgre}
        \end{figure}
...
\end{lstlisting}
          \end{minipage}
          \caption{Aktivasi ekstensi pgvector pada PostgreSQL}
          \label{lst:pgvector-enable}
        \end{figure}
...
\end{lstlisting}
          \end{minipage}
          \caption{Import dependencies untuk implementasi RAG Service}
          \label{lst:rag-imports}
        \end{figure}
...
private async initializeRAG() {
  ...
  const schema = this.configService.get('DATABASE_SCHEMA');
  const table = this.configService.get('DATABASE_TABLE_NAME') ;
...
\end{lstlisting}
          \end{minipage}
          \caption{Implementasi semantic chunking pada LoadDocumentsProvider}
          \label{lst:load-docs-impl}
        \end{figure}

        \hspace*{1.25cm}Proses pemuatan dokumen dijalankan oleh komponen \texttt{LoadDocumentsProvider} yang menerapkan strategi \textit{semantic chunking} yang ditunjukkan pada Gambar \ref{lst:load-docs-impl}. Strategi ini memecah data berdasarkan unit makna yang utuh, bukan berdasarkan jumlah karakter atau \textit{token}. Setiap \textit{file} JSON dibaca menggunakan modul \texttt{fs} Node.js, kemudian dipecah menjadi beberapa dokumen berdasarkan jenis informasinya: ringkasan sentimen keseluruhan (\texttt{sentimentOverall}), faktor-faktor positif (\texttt{faktorPositifTop10}), dan faktor-faktor negatif (\texttt{faktorNegatifTop10}). Setiap \textit{chunk} informasi kemudian dibungkus dalam objek \texttt{Document} dari LangChain.js, yang menyediakan struktur standar dengan properti \texttt{pageContent} untuk menyimpan teks utama dan \texttt{metadata} untuk informasi tambahan.

        \hspace*{1.25cm}Setiap dokumen yang dihasilkan dilengkapi dengan \textit{metadata} yang berperan penting dalam memperkaya konteks sekaligus mempermudah proses \textit{retrieval}. Pemisahan ini sesuai dengan rancangan pada Bab 3 yang menyatakan bahwa \textit{metadata} membantu meningkatkan akurasi pencarian karena memungkinkan sistem melakukan penyaringan dokumen sesuai dengan konteks pertanyaan pengguna. \\

        \hspace*{1.25cm}Selanjutnya, fase \textit{query} berhasil diimplementasikan pada Gambar \ref{lst:query-chain-impl} untuk menangani keseluruhan proses sejak penerimaan pertanyaan hingga generasi jawaban, sesuai dengan rancangan RAG \textit{Query Pipeline} pada Bab 3. Implementasi fase ini memanfaatkan \textit{LangChain Expression Language (LCEL)} yang menyusun alur eksekusi sebagai rangkaian proses berurutan dalam bentuk \texttt{RunnableSequence}. Dengan LCEL, setiap tahapan didefinisikan sebagai komponen modular yang saling terhubung, mulai dari \textit{retriever} untuk pencarian dokumen, \textit{prompt template} untuk penyusunan konteks, model LLM untuk generasi jawaban, hingga \textit{output parser} untuk mengekstrak teks jawaban dari respons API.
...
Anda adalah asisten analisis sentimen UMKM.
...
\end{lstlisting}
          \end{minipage}
          \caption{Implementasi RAG Query Chain menggunakan LCEL}
          \label{lst:query-chain-impl}
        \end{figure}
