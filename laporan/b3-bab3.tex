...
\subsection{Alur Pelaksanaan Penelitian}
...
\begin{enumerate}
	...
	\item \textbf{Use Case Diagram}
	      ...
	      \begin{figure}[h]
		      \centering
		      \includegraphics[width=0.8\textwidth]{use case baru lagi 1.png}
	...
	\item \textbf{Pipeline Retrieval-Augmented Generation (RAG)}
	      ...
	      \hspace*{1.25cm}Pipeline RAG dapat dikatakan sebagai inti dari sistem chatbot yang dikembangkan dalam penelitian ini. Komponen ini dirancang untuk mengatasi kelemahan \textit{Large Language Model} konvensional yang cenderung menghasilkan \textit{hallucination}, yaitu jawaban yang terdengar meyakinkan namun tidak sepenuhnya akurat, terutama ketika berhadapan dengan pengetahuan khusus yang tidak tersedia dalam data pelatihan model. Dengan menambahkan mekanisme pencarian informasi (\textit{retrieval}), sistem tidak hanya bergantung pada pengetahuan bawaan model, tetapi juga dapat mengambil informasi faktual dari basis pengetahuan (\textit{knowledge base}) yang berisi hasil analisis sentimen UMKM. Informasi yang ditemukan kemudian digunakan sebagai konteks dalam proses pembuatan jawaban (\textit{generation}), sehingga jawaban yang dihasilkan menjadi lebih relevan, lebih akurat, dan memiliki dasar data yang jelas.

	      \hspace*{1.25cm}Dalam implementasinya, pipeline RAG pada penelitian ini dibangun menggunakan LangChain.js sebagai \textit{framework} pengatur utama. LangChain.js dipilih karena menyediakan komponen-komponen siap pakai yang mempermudah penyatuan berbagai bagian RAG. Komponen tersebut meliputi \textit{document loaders} untuk memuat data dari berkas JSON, \textit{text splitters} untuk memecah dokumen menjadi bagian-bagian kecil, \textit{embedding models} untuk mengubah teks menjadi representasi angka berbentuk vektor, \textit{vector stores} untuk menyimpan dan mencari vektor tersebut, \textit{retrievers} untuk melakukan pencarian berdasarkan kemiripan makna, hingga \textit{chains} yang mengatur alur kerja kompleks antara proses pencarian dan pembuatan jawaban. \textit{Framework} ini juga mendukung integrasi yang mulus dengan berbagai penyedia \textit{Large Language Model} dan model \textit{embedding}, sehingga memberikan fleksibilitas dalam memilih teknologi sesuai kebutuhan sistem. Selain itu, LangChain.js menggunakan \textit{LangChain Expression Language (LCEL)} yang memungkinkan pembangunan pipeline secara terstruktur dan modular, sehingga memudahkan pemeliharaan dan pengembangan sistem di masa mendatang.

	      \hspace*{1.25cm}Pipeline RAG pada penelitian ini terbagi menjadi dua tahap utama yang berjalan terpisah namun saling melengkapi. Tahap pertama adalah RAG \textit{Ingestion Pipeline}, yaitu proses yang dijalankan ketika ada data baru yang perlu dimasukkan ke dalam basis pengetahuan, misalnya hasil analisis sentimen terbaru atau pembaruan data dari media sosial. Pada tahap ini, data diolah dan disiapkan agar dapat digunakan secara efektif dalam mekanisme pencarian. Tahap kedua adalah RAG \textit{Query Pipeline}, yang aktif setiap kali pengguna mengajukan pertanyaan melalui chatbot. Pada tahap ini, sistem akan mencari informasi yang paling relevan dari basis pengetahuan berdasarkan pertanyaan pengguna, kemudian menggabungkannya sebagai konteks untuk menghasilkan jawaban. Kombinasi kedua tahap ini memungkinkan chatbot tidak hanya memberikan respons yang terdengar alami, tetapi juga tetap berlandaskan pada data yang spesifik dan sesuai dengan kebutuhan pengguna UMKM.


	      \begin{enumerate}

		      \item RAG Ingestion Pipeline merupakan tahap awal yang memegang peran penting dalam keseluruhan sistem \textit{Retrieval-Augmented Generation (RAG)}. Pipeline ini bertugas menyiapkan \textit{knowledge base} yang nantinya menjadi rujukan utama chatbot dalam menjawab pertanyaan pengguna. Perancangan pipeline ini berfokus pada bagaimana data hasil analisis sentimen Instagram UMKM dapat diproses secara rapi, terstruktur, dan siap digunakan untuk pencarian berbasis makna. Setiap tahapan dijalankan secara berurutan agar kualitas data yang tersimpan tetap konsisten dan optimal ketika digunakan pada proses \textit{retrieval}. Implementasi pipeline ini memanfaatkan komponen-komponen LangChain.js seperti \texttt{JSONLoader} untuk membaca file JSON, \texttt{Document} untuk representasi data terstruktur, \texttt{GoogleGenerativeAIEmbeddings} untuk konversi teks ke vektor, dan \texttt{PGVectorStore} untuk penyimpanan dan pencarian vektor di PostgreSQL.
		      ...
		      \item RAG Query Pipeline merupakan rangkaian proses yang dijalankan ketika pengguna mengajukan pertanyaan melalui antarmuka chatbot. Pipeline ini dirancang untuk menghasilkan jawaban yang cepat, relevan, dan akurat dengan memanfaatkan knowledge base yang telah dipersiapkan sebelumnya melalui RAG Ingestion Pipeline. Seluruh tahapan dalam pipeline ini saling terhubung dan dijalankan secara berurutan untuk memastikan bahwa setiap pertanyaan diproses dengan konteks yang tepat sebelum dikirim ke model \textit{large language model}. Implementasi query pipeline menggunakan \textit{LangChain Expression Language (LCEL)} yang memungkinkan konstruksi alur kerja secara deklaratif dengan \texttt{RunnableSequence}, menggabungkan retriever, prompt template, LLM, dan output parser dalam satu chain yang kohesif dan mudah dipelihara.
		            ...
		            \begin{enumerate}
			            ...
			            \item Tahap Query Embedding, setelah proses \textit{preprocessing} selesai, pertanyaan pengguna diubah menjadi representasi vektor numerik menggunakan model \textit{gemini-embedding-001}, yaitu model yang sama dengan yang digunakan pada tahap ingestion data. Transformasi ini dilakukan karena sistem tidak dapat memproses teks secara langsung dalam bentuk kata, melainkan perlu direpresentasikan dalam bentuk angka agar dapat dibandingkan dengan dokumen yang tersimpan di database vektor. Penggunaan model embedding yang konsisten sangat krusial untuk memastikan bahwa pertanyaan pengguna dan dokumen berada dalam ruang vektor yang sama, sehingga perhitungan kemiripan dapat dilakukan secara akurat dan bermakna. LangChain.js menangani proses ini melalui method \texttt{embedQuery()} pada instance \texttt{GoogleGenerativeAIEmbeddings}, yang secara otomatis mengonversi string pertanyaan menjadi vektor dengan dimensi yang sama dengan dokumen di knowledge base.

			            \item Tahap Similarity Search and Retrieval, pada tahap ini sistem mulai mencari informasi yang paling relevan dengan pertanyaan pengguna. Pencarian dilakukan dengan membandingkan vektor query pengguna dengan vektor dokumen yang telah tersimpan di PostgreSQL menggunakan ekstensi \textit{pgvector}. Metode yang digunakan adalah \textit{cosine similarity}, yang mengukur kedekatan makna berdasarkan sudut antar vektor, sehingga lebih stabil terhadap perbedaan panjang teks atau gaya penulisan. Sistem kemudian menerapkan strategi \textit{Top-K retrieval} dengan nilai K = 3, yang berarti sistem hanya mengambil tiga dokumen paling relevan. Pemilihan nilai tiga dokumen dipilih berdasarkan jumlah dokumen / \textit{chunk} yang ada. tiga dokumen mampu memberikan keseimbangan yang cukup baik antara kualitas konteks dan efisiensi sistem. Pemilihan nilai ini merupakan trade-off antara kualitas jawaban dan efisiensi sistem, karena terlalu sedikit dokumen bisa membuat jawaban kurang lengkap, sementara terlalu banyak dokumen dapat meningkatkan token api dan memperlambat proses. LangChain.js menyediakan interface \texttt{VectorStoreRetriever} yang dapat dikonfigurasi melalui method \texttt{asRetriever(\{k: 3\})} pada \texttt{PGVectorStore}, yang secara otomatis menangani query embedding, similarity search dengan operator \texttt{<=>} untuk cosine distance, dan pengembalian top-K dokumen beserta metadata dan similarity score.

			            \item Tahap Prompt Construction, pada tahap ini dokumen-dokumen yang berhasil ditemukan tidak langsung diberikan kepada model, tetapi terlebih dahulu disusun menjadi sebuah prompt yang terstruktur dan mudah dipahami oleh \textit{Large Language Model (LLM)}. Prompt ini berfungsi sebagai panduan bagi model dalam menghasilkan jawaban. Di dalamnya terdapat instruksi sistem yang menjelaskan bahwa model berperan sebagai asisten analisis sentimen UMKM, konteks berupa dokumen hasil retrieval yang berisi data sentimen aktual, serta aturan penggunaan bahasa Indonesia yang formal namun tetap komunikatif bagi pelaku UMKM. Pertanyaan asli pengguna diletakkan di bagian akhir agar model tetap fokus pada kebutuhan utama pengguna saat menghasilkan respons. Implementasi menggunakan \texttt{PromptTemplate} dari LangChain.js dengan template string yang mendefinisikan struktur prompt, variabel \texttt{\{context\}} untuk menampung dokumen hasil retrieval, dan variabel \texttt{\{question\}} untuk pertanyaan pengguna.

			            \item Tahap Answer Generation, pada tahap ini prompt yang telah disusun dikirim ke layanan Groq AI untuk diproses oleh model \textit{openai/gpt-oss-20b}. Groq dipilih karena banyaknya model yang tersedia dengan harga yang terjangkau dan bisa saja menjadi gratis jika bisa memanajemen token dengan baik. Model ini juga memiliki kemampuan yang baik dalam memahami konteks kompleks, menafsirkan data numerik, serta menyusun jawaban yang informatif berdasarkan informasi yang diberikan. LangChain.js mengintegrasikan Groq melalui class \texttt{ChatGroq} yang menangani komunikasi dengan Groq API, streaming response, serta pengelolaan parameter seperti temperature dan max tokens. Output dari LLM kemudian diproses oleh \texttt{StringOutputParser} untuk mengekstrak teks jawaban dari struktur response API.
...
\begin{enumerate}
	...
	\item \textbf{Implementasi Pipeline RAG}
	      ...
	      \hspace*{1.25cm}Pipeline \textit{Retrieval-Augmented Generation} diimplementasikan dalam dua fase utama, yaitu fase \textit{ingestion} dan fase \textit{query}, yang terintegrasi menggunakan LangChain.js sebagai \textit{framework} pengatur alur kerja. Pada fase ingestion, sistem memuat data hasil analisis sentimen dari file JSON dan tidak langsung menyimpannya ke database, melainkan terlebih dahulu mengolahnya menjadi dokumen teks yang terstruktur. Pengelompokan dilakukan berdasarkan kategori informasi yang relevan seperti ringkasan sentimen, faktor positif, dan faktor negatif, sehingga setiap dokumen merepresentasikan satu unit pengetahuan yang utuh dan mudah ditelusuri. Melalui komponen \textit{document loader} dan objek \textit{Document} pada LangChain.js, data mentah dapat diubah menjadi representasi terstruktur yang dilengkapi metadata untuk mendukung proses pencarian berbasis makna.

	      \hspace*{1.25cm}Setiap dokumen yang telah terbentuk kemudian dikonversi menjadi \textit{embedding} menggunakan \textit{embedding model} yang diakses melalui integrasi LangChain.js. \textit{Embedding} tersebut disimpan ke dalam \textit{vector store} berbasis PostgreSQL yang dilengkapi dengan ekstensi \textit{pgvector} melalui komponen \textit{PGVectorStore}. Proses penyimpanan dirancang bersifat \textit{idempotent}, sehingga dokumen yang sama tidak akan disimpan ulang apabila sudah tersedia di dalam database. Pendekatan ini meningkatkan efisiensi sistem, terutama ketika proses ingestion dijalankan kembali tanpa adanya perubahan data, sekaligus menjaga konsistensi dan integritas \textit{knowledge base}.

	      \hspace*{1.25cm}Fase query berjalan setiap kali pengguna mengajukan pertanyaan melalui chatbot. Pertanyaan pengguna diproses dan diubah menjadi \textit{embedding} menggunakan metode \textit{embedQuery()} dari LangChain.js, kemudian dibandingkan dengan vektor dokumen di database menggunakan mekanisme \textit{similarity search}. Dokumen dengan kemiripan tertinggi dipilih melalui \textit{retriever} dengan pendekatan \textit{Top-K retrieval}, lalu disusun menjadi \textit{prompt} terstruktur menggunakan \textit{PromptTemplate} sebelum dikirimkan ke \textit{Large Language Model}. Dengan dukungan \textit{LangChain Expression Language (LCEL)}, seluruh rangkaian proses ini dirangkai dalam satu \textit{chain} yang modular, sehingga respons yang dihasilkan tidak hanya bersifat umum, tetapi juga kontekstual dan berbasis pada data analisis sentimen yang tersedia.
